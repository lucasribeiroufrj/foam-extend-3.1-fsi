...
defineTypeNameAndDebug(nonNewtonianIcoFlow, 0);
addToRunTimeSelectionTable(flowModel, nonNewtonianIcoFlow, dictionary);


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

nonNewtonianIcoFlow::nonNewtonianIcoFlow(const fvMesh& mesh)
:
...
const volVectorField& nonNewtonianIcoFlow::U() const
{
    return U_;
}


const volScalarField& nonNewtonianIcoFlow::p() const
{
    return p_;
}
...
tmp<vectorField> nonNewtonianIcoFlow::patchViscousForce(const label patchID) const
{
    tmp<vectorField> tvF
    (
    ...
    tvF() = rho_.value()*fluid_.nu().boundaryField()[patchID]*U().boundaryField()[patchID].snGrad();
...
tmp<scalarField> nonNewtonianIcoFlow::patchPressureForce(const label patchID) const
{
    tmp<scalarField> tpF
    (
...
//- Patch viscous force (N/m2)
tmp<vectorField> nonNewtonianIcoFlow::faceZoneViscousForce
(
    const label zoneID,
    const label patchID
) const
...
//- Patch pressure force (N/m2)
tmp<scalarField> nonNewtonianIcoFlow::faceZonePressureForce
(
    const label zoneID, 
    const label patchID
) const
...
tmp<scalarField> nonNewtonianIcoFlow::faceZoneMuEff
(
    const label zoneID,
    const label patchID                
) const
{   
    scalarField pME = patchMuEff(patchID);
    ...
    // Parallel data exchange: collect mu (viscosity) field on all processors
    reduce(muEff, sumOp<scalarField>());

    return tmuEff;
}

tmp<scalarField> nonNewtonianIcoFlow::patchMuEff(const label patchID) const
{
    tmp<scalarField> tmE
    (
        new scalarField(mesh().boundary()[patchID].size(), 0)
    ...
    tmE() = rho_.value()*fluid_.nu().boundaryField()[patchID];

    return tmE;
}

void nonNewtonianIcoFlow::evolve()
{
